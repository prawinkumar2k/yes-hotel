...
**Actually run on real GitHub Actions infrastructure, not just written**: pushed to `github.com/prawinkumar2k/yes-hotel`, and the workflow triggered automatically. The **first real run failed** — `pnpm/action-setup@v4` errors when both an explicit `version:` input and `package.json`'s own `packageManager` field are set (a documented conflict neither local testing nor code review would ever surface, since it's specific to how that action behaves on GitHub's runners). Fixed by dropping the redundant `version:` input. While fixing it, reasoning through the config surfaced a second, deeper bug before it could fail a second run: GitHub Actions' `services:` block has no way to pass `--replSet` to the `mongo:7` container's entrypoint, so `rs.initiate()` would have failed next regardless — replaced with a manually-started container (`docker run ... mongo:7 --replSet rs0`) that has full control over the command line. **The second run, after both fixes, passed completely** — every single step in both jobs succeeded, including the production-artifact-boot check and the full Playwright/accessibility suite. Required secrets for a real run: none for the CI job itself (it uses ephemeral throwaway values); a real deploy would need the full `.env.example` list as repository/environment secrets.
